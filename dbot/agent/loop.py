...
from dbot.agent.tools.filesystem import EditFileTool, ListDirTool, ReadFileTool, WriteFileTool
...
from dbot.agent.tools.web import WebFetchTool, WebSearchTool
...
if TYPE_CHECKING:
    from dbot.config.schema import ChannelsConfig, ExecToolConfig
...
class AgentLoop:
    ...
    def __init__(
        ...
        from dbot.config.schema import ExecToolConfig
        self.bus = bus
        ...
        self._consolidating: set[str] = set()  # 正在进行合并的会话键
        self._consolidation_tasks: set[asyncio.Task] = set()  # 进行中任务的强引用
    ...
    async def _connect_mcp(self) -> None:
        """连接已配置的 MCP 服务器（一次性，延迟连接）。"""
        if self._mcp_connected or self._mcp_connecting or not self._mcp_servers:
            return
        self._mcp_connecting = True
        from dbot.agent.tools.mcp import connect_mcp_servers
        try:
            ...
            logger.error("连接 MCP 服务器失败（将在下一条消息时重试）：{}", e)
            if self._mcp_stack:
                try:
        ...
        def _fmt(tc):
            ...
            for key, value in args.items():
                if isinstance(value, str):
                    ...
                    arg_parts.append(f'{key}=None')
                else:
                    # 复杂类型用 JSON 格式
                    arg_parts.append(f'{key}={json.dumps(value, ensure_ascii=False)}')
            return f'{tc.name}({", ".join(arg_parts)})'
    ...
    async def _run_agent_loop(
        ...
        """运行 Agent 迭代循环。返回 (final_content, tools_used, messages)。"""
        ...
        while iteration < self.max_iterations:
            ...
            if response.has_tool_calls:
                ...
                # 不要将错误响应持久化到会话历史中 — 它们会
                # 污染上下文并导致永久 400 循环 (#1303)。
        ...
        if final_content is None and iteration >= self.max_iterations:
            logger.warning("已达到最大迭代次数 ({})", self.max_iterations)
            final_content = (
    ...
    async def _dispatch(self, msg: InboundMessage) -> None:
        ...
        async with self._processing_lock:
            try:
                ...
                logger.exception("处理会话 {} 的消息时出错", msg.session_key)
                await self.bus.publish_outbound(OutboundMessage(
                    channel=msg.channel, chat_id=msg.chat_id,
    ...
    async def _process_message(
        ...
        logger.info("响应发送到 {}:{}: {}", msg.channel, msg.sender_id, preview)
        return OutboundMessage(
