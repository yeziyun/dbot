...
from dbot.providers.registry import find_by_model, find_gateway
...
def _short_tool_id() -> str:
    """生成一个与所有提供商兼容的 9 字符字母数字 ID（包括 Mistral）。"""
...
class LiteLLMProvider(LLMProvider):
    ...
    通过统一的接口支持 OpenRouter、Anthropic、OpenAI、Gemini、MiniMax 和许多其他提供商。
    提供商特定的逻辑由注册表驱动
    （参见 providers/registry.py）——这里不需要 if-elif 链。
    """
    ...
    def _sanitize_messages(messages: list[dict[str, Any]], extra_keys: frozenset[str] = frozenset()) -> list[dict[str, Any]]:
        """删除非标准键并确保助手消息具有 content 键。"""
        allowed = _ALLOWED_MSG_KEYS | extra_keys
        sanitized = []
        for msg in messages:
            clean = {k: v for k, v in msg.items() if k in allowed}
            # 严格的提供商即使在助手只有 tool_calls 时也需要 "content"
    ...
    async def chat(
        ...
        # 应用模型特定的覆盖（例如 kimi-k2.5 temperature）
